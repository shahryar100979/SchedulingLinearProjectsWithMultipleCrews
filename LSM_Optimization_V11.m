...
validation_only          = true  ; % boolean for deriving the results of Dr. Elrayes 2018 paper
if validation_only
...
NP = NT ; % number of dummy temporary dismissal unit for each crew
...
M_R = 15  ; % maximum duration of interuption at each unit in days
...
for k = 1:K
    for it_crew = 1:num_crews(k)
        for i = 1:NT
            for j = 1:NI
                cons1k(count) = it_NT(i,k,it_crew) >= M*( 1-sum(x_NI_NT(j,i,k,it_crew)) ) + 1 ;
                count = count + 1 ;
            end
        end
    end
...
for k = 1:K
    for it_crew = 1:num_crews(k)
        for i = 1:NP
            for j = 1:NT
                cons1l(count) = pt_NT(j,k,it_crew) >= M*( 1-sum(x_NT_NP(j,i,k,it_crew)) ) + T_min + 1  ;
                count = count + 1 ;
            end
        end
    end
...
for k = 1:K
    for it_crew = 1:num_crews(k)
        for i = 1:NI
            for j = 1:NI
                if i == j; continue; end
                cons2a(count) = xs_NI(i,k,it_crew)  + Quantity(k,i)/Productivity(k,it_crew) - ...
...
for k = 1:K
    for it_crew = 1:num_crews(k)
        for i = 1:NI
            for j = 1:NT
                cons2c(count) = xs_NI(i,k,it_crew)  + Quantity(k,i)/Productivity(k,it_crew) - ...
