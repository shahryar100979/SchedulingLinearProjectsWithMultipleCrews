...
if validation_only
    ...
    daily_indirect_cost = 5000 ; % from paper "Optimization model for resource assignment problems of linear construction projects"
end

M_T = 500 ; % a large number that is >>> project duration
M_R = 20  ; % maximum duration of interuption at each unit in days

time_limit = 60*20 ; % optimization time limit (seconds)
...
options = optimoptions('intlinprog','MaxTime',time_limit, 'ConstraintTolerance', 1.0e-4)  ;
...
LS_Problem_modified.Objective = interruption_cost + dismissal_cost + indirect_cost ;
...
cons6a(1) = LS_Problem_modified.Objective <= ...
    evaluate(LS_Problem_modified.Objective , sol_min_duration_min_cost) ;
...
cons10a    = optimconstr(1) ;
cons10a(1) = sum(sum(sum(it_NT))) >= 100 ;
LS_Problem.Constraints.cons10a = cons10a ;
sol     = solve(LS_Problem,sol) ;
