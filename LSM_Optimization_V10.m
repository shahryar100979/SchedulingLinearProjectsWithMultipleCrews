...
if validation_only
    daily_indirect_cost = 5000 ; %#ok<UNRCH>  % to derive El-rayes results a high indirect cost is set to ensure the duration of project is minimized
else
    daily_indirect_cost = 50000 ; % from paper "Optimization model for resource assignment problems of linear construction projects"
end
...
cons6a(1) = LS_Problem_modified.Objective <= ...
    evaluate(LS_Problem_modified.Objective , sol)*1.001 ;
...
LS_Problem_modified.Objective = number_of_interruption ;
LS_Problem_modified.Constraints.cons6a = cons6a ;
sol     = solve(LS_Problem_modified,sol,'options',options) ;
...
cons8a(1) = LS_Problem_modified.Objective <= ...
    evaluate(LS_Problem_modified.Objective , sol)*1.00001 ;
...
LS_Problem_modified.Objective = total_idle_time ;
LS_Problem_modified.Constraints.cons8a = cons8a ;
sol     = solve(LS_Problem_modified,sol,'options',options) ;
